...
import com.example.systemposApp.repository.UserRepository;
...
public class UserServiceImpl implements UserService{
	...
	private boolean checkUsernameAvailable(User user) throws Exception {
		...
		if (userFound.isPresent()) {
			throw new CostumeFieldValidationException("Username no disponible","username");
		}
		return true;
	}
	...
	private boolean checkPasswordValid(User user) throws Exception {
		if (user.getConfirmPassword() == null || user.getConfirmPassword().isEmpty()) {
			throw new CostumeFieldValidationException("Confirm Password es obligatorio","confirmPassword");
		}
		
		if ( !user.getPassword().equals(user.getConfirmPassword())) {
			throw new CostumeFieldValidationException("Password y Confirm Password no son iguales","password");
		}
		return true;
	}
